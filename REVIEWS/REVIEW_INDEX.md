...
| REVIEW-2026-05-16-pr-8-boot-031-scaffold-extract-tool | PR #8 BOOT-031 scaffold extraction tool | request changes; review-fix applied; fresh-context re-review pending | high | Codex adversarial review | 2026-05-16 | `REVIEWS/REVIEW-2026-05-16-pr-8-boot-031-scaffold-extract-tool.md` | active; P1 extracted-registry coverage and reset-file shapes addressed in `SCRIPTS/scaffold-extract.sh` (10 new registry rows including the four reviewer-flagged framework paths plus `GOVERNANCE_PERFORMANCE.md`, `ARTIFACTS/.gitkeep`, `ARTIFACTS/ARCHIVE/.gitkeep`, `00_intake/raw/.gitkeep`, `00_intake/summaries/.gitkeep`; header-only tables for `STALE_ITEMS.md`, `ADR/ADR_INDEX.md`, `HANDOFFS/HANDOFF_INDEX.md`). P2 addressed by adding `case_scaffold_extract_registry_includes_kept_framework_paths` and `case_scaffold_extract_reset_files_use_header_only_tables` to `SCRIPTS/validate-bootstrap-red-checks.sh`. P3 addressed by removing the inaccurate "governance performance log" wording from the script's usage text. Fresh-context Codex re-review pending after push and GitHub `validate`. |
